...
def getInventory(session):
    ...
    my_inventory = session.getInventory()
    # print my_inventory
    # logging.info(my_inventory)
    return my_inventory.to_dict()
...
if __name__ == '__main__':
    ...
    if session:
        ...
        # General
        # getProfile(session)
        # my_inventory = getInventory(session)
        # pokemon_list =  my_inventory['party']
        # print pokemon_list
        ...
        inventory = session.checkInventory()
        pokemon_list = inventory.to_dict()['party']


        # code for releasing bad pokemons
        bad_pokemon_ids = []
        if len(bad_pokemon_ids) == 0:
            bad_pokemon_ids = find_bad_pokemons(pokemon_list)
        for pokemon in inventory.party:        
            if pokemon.id in bad_pokemon_ids:                    
                if SHOULD_RELEASE:
                    print "releasing"
                    print pokemon
                    res = session.releasePokemon(pokemon)                
                    time.sleep(1)
                else:
                    print pokemon

        # code for printing pokemon info
        if SHOULD_GENERATE_SUMMARY:
            pokemon_summary = generate_pokemon_list_summary(pokemon_list)
            print summary_to_readable(pokemon_summary)
